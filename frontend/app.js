...
const newsletterForm = document.getElementById("newsletter-form"), newsletterStatus = document.getElementById("newsletter-status"), newsletterEmail = document.getElementById("newsletter-email");
newsletterForm && newsletterForm.addEventListener("submit", async e => { e.preventDefault(); const t = { email: newsletterEmail.value }; try { e.submitter && (e.submitter.disabled = !0); const n = await fetch("https://zq7qfmhra1.execute-api.eu-west-3.amazonaws.com/prod/subscribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(t) }), s = await n.json().catch(() => ({})); n.ok ? (newsletterStatus.textContent = "Inscription réussie ! Un email de confirmation vous a été envoyé.", newsletterStatus.className = "text-sm font-medium text-center py-3 px-4 rounded-xl text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 block", newsletterForm.reset()) : (newsletterStatus.textContent = s.error || "Erreur lors de l'inscription.", newsletterStatus.className = "text-sm font-medium text-center py-3 px-4 rounded-xl text-rose-400 bg-rose-400/10 border border-rose-400/20 block") } catch (e) { newsletterStatus.textContent = "Erreur réseau. Impossible de contacter le serveur.", newsletterStatus.className = "text-sm font-medium text-center py-3 px-4 rounded-xl text-rose-400 bg-rose-400/10 border border-rose-400/20 block" } finally { e.submitter && (e.submitter.disabled = !1) } });
