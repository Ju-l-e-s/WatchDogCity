...
function escapeRegExp(string) {
    return string.replace(/[.*+?^${}()|[\]\\]/g, "\\function escapeHTML");
}
        ...
        councilSection.innerHTML = `
            ...
            <div class="bg-white rounded-[2rem] shadow-card overflow-hidden mt-5 md:mt-0">
                <div class="px-4 pt-5 pb-[10px] md:px-12 md:py-10 border-b border-slate-100/60">
                    ...
                    <p class="text-slate-600 leading-relaxed max-w-3xl text-base font-light">${escapedSummary}</p>
                </div>
...
function renderDeliberationRow(d) {
    ...
    const unanimityBadge = isUnanimous ? `<span class="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full ml-2">Unanimité</span>` : '';
    ...
    const voteWidget = hasVote
        ? `<div class="bg-white rounded-2xl p-6 shadow-micro border border-slate-100/80">
            ...
            ${isUnanimous
                ? `<div class="flex items-center gap-3"><div class="h-1.5 flex-1 bg-emerald-500 rounded-full"></div><span class="text-sm font-semibold text-emerald-700">${d.vote.pour} pour</span></div>`
        ...
        <button onclick="toggleDelib('${delibId}')" id="btn-${delibId}" aria-expanded="false" aria-controls="content-${delibId}" class="delib-trigger w-full text-left px-4 py-4 md:px-8 md:py-6 flex items-center justify-between gap-4 min-h-[64px]">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2.5 mb-2">
                    ...
                    <span class="text-[11px] font-medium text-slate-600 uppercase tracking-widest">${highlightedTag}</span>
                    ${isUnanimous ? '<span class="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded">Unanime</span>' : ''}
                </div>
                    ...
                    <div class="px-4 pt-5 pb-5 md:px-10 md:py-8">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            <div class="lg:col-span-7">
                                ...
                                ${analysisHTML}
                                ${d.disagreements && d.disagreements !== 'null' ? `<div class="border-l-2 border-slate-200 pl-5 py-1 mb-8"><h5 class="text-[11px] font-semibold text-slate-600 uppercase tracking-widest mb-2">Opposition</h5><p class="text-[15px] text-slate-600 italic leading-relaxed">${highlightText(d.disagreements, searchQuery, d.acronyms)}</p></div>` : ''}
...
function renderVoteBar(l, v, t, c) {
    ...
    return `<div><div class="flex justify-between text-[11px] font-medium mb-1"><span class="text-slate-600">${l}</span><span class="text-slate-600 font-semibold">${v}</span></div><div class="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div class="${c} h-full rounded-full transition-all duration-500" style="width:${p}%"></div></div></div>`;
}
